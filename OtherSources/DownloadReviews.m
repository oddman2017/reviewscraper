...
{	
	NSString *urlString = [NSString stringWithFormat:@"http://phobos.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?sortOrdering=4&onlyLatestVersion=false&sortAscending=true&pageNumber=%d&type=Purple+Software&id=%@", (int)pageNumber, appCode];
	
	NSURL *url = [NSURL URLWithString:urlString];
	NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:60.0];
	[request addValue:[NSString stringWithFormat:@"%d-1", itunesCode] forHTTPHeaderField:@"X-Apple-Store-Front"];
...
{	
	...
	if([xml length] > 54)
	{		
		ParseXML *tidied = [[ParseXML alloc] init];
